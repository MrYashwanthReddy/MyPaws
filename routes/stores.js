...
router.route("/").get(async (req, res) => {
  try {
    ...
    const config = {
      method: "get",
      url: `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${searchConfig.HobokenLocation}&radius=${searchConfig.radius}&type=pet_store&key=AIzaSyCCBTWUmMlVv-dt5VvetyGkH9mdbVL6iy4`,
      headers: {},
    };
  ...
  .post(async (req, res) => {
    try {
      ...
      if (reviewId === "0") {
        ...
        if (result.insertedReview) return res.redirect(`/pet-store/review/${placeId}`);
      } else {
        const result = await reviews.updateReview(reviewId, data);
        if (result.updatedReview) return res.redirect(`/pet-store/review/${placeId}`);
      }
    } catch (error) {
      res.status(error.status).json({error: error.msg});
    }
...
router.route("/review-delete/:placeId/:reviewId").get(async (req, res) => {
  try {
    ...
    if (result.deletedReview) return res.redirect(`/pet-store/review/${placeId}/`);
  } catch (error) {
    res.status(error.status).json({error: error.msg});
  }
});
